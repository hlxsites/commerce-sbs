...
const getPath = async (key, rootCategoryKey) => {
  ...
  let currentCategory;
  try {
    currentCategory = await fetchGet(endpoint, storeView, getCategory, {
      uid: key,
    });
  } catch (err) {
...
const getItems = async (folderKey) => {
  ...
  try {
    ...
    const categories = await fetchGet(
      endpoint,
      storeView,
    ...
    categories?.data?.categories?.items.forEach((category) => {
      newItems[category.uid] = {
        ...
        key: category.uid,
        childCount:
          parseInt(category.children_count) + parseInt(category.product_count),
      };
    });
  } catch (err) {
