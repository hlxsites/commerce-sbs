...
import { PLACEHOLDER_IMG } from '../commerce.js';
import { store } from './init-cart.js';
import { loadCSS } from '../lib-franklin.js';
import { getCart, removeItemFromCart, updateQuantityOfCartItem } from './api.js';
    ...
    return html`<li>
      <div class="minicart-product">
        ...
        <div class="info">
          ...
          <div class="options">
            <input type="checkbox" id="see-options-1" />
            <label for="see-options-1">See Details</label>
            <dl>
        ...
        ${state.confirmDelete && html`<${ConfirmDeletionOverlay} close=${() => this.setState({ confirmDelete: false })} confirm=${() => this.props.api.removeItemFromCart(item.uid)} />`}
      </div>
    </li>`;
  }
}
  ...
  render(props, state) {
    ...
    if (!cart.items || cart.items.length === 0) {
      return html`<div class="minicart-panel empty">
        <div class="minicart-header">
          <button class="close" onClick=${() => close(false)}>${cart.total_quantity}</button>
        </div>
    ...
    return html`<div class="minicart-panel">
      ...
      <ul class="minicart-list">
        ${state.cart.items.map((item) => html`<${ProductCard} item=${item} formatter=${this.formatter} api=${props.api} />`)}
      </ul>
...
export async function toggle(refetch = true) {
  if (!cartVisible) {
    // Load CSS
    await new Promise((resolve) => { loadCSS('/styles/minicart.css', resolve); });
